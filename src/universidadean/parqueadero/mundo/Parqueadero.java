...
public class Parqueadero {
    ...
    public String metodo1() {
        ...
        String placa = "No hay ningun carro parqueado";
        int horaMenor = 0;
        ...
        for (int i = 0; i < c.size(); i++) {
            if(horaMenor == 0) {
                horaMenor = c.get(i).darHoraLlegada();
                placa = c.get(i).darPlaca();
            }

            if (c.get(i).darHoraLlegada() < horaMenor) {
                horaMenor = c.get(i).darHoraLlegada();
                placa = c.get(i).darPlaca();
            }
        }
        ...
        if (horaMenor != 0) {
            placa = "El carro que lleva más tiempo es el de placas\n" + placa + "\nHora de ingreso: " + horaMenor;
        }
    ...
    public String metodo2() {
        int totalCupos = puestos.length;
        int libres = calcularPuestosLibres();

        String respuesta = "Hay puestos disponibles: " + libres;
        if (libres == 0) {
